...
from ..core.models import LogEntry, Anomaly, AnalysisResult
...
class ProcessingStats:
    ...
    def update(self, count: int, elapsed: float):
        ...
        self.avg_time_per_entry = self.total_time / self.total_processed if self.total_processed > 0 else 0
...
class BatchProcessor:
    ...
    def process_entries(self, entries: List[LogEntry], 
                       processor: Callable[[LogEntry], Any]) -> List[Any]:
        """
...
class AsyncBatchProcessor:
    ...
    async def process_entries(self, entries: List[LogEntry],
        ...
        logger.info("async_batch_processed",
                   count=len(entries),
                   elapsed=f"{elapsed:.2f}s",
                   throughput=f"{self.stats.throughput:.2f} entries/s")
...
class ParallelProcessor:
    ...
    def process_entries(self, entries: List[LogEntry],
        ...
        logger.info("parallel_processed",
                   count=len(entries),
                   elapsed=f"{elapsed:.2f}s",
