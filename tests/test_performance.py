...
from loggem.performance import (
    ...
    AnalysisCache, cached_analysis, ProcessingStats, AdaptiveBatcher
)
...
def test_cache_eviction():
    ...
    entries = [
        LogEntry(timestamp=datetime.now(), source="test", 
                message="test", raw=f"Entry {i}", metadata={})
        for i in range(3)
    ]
...
def test_cache_clear():
    ...
    entry = LogEntry(timestamp=datetime.now(), source="test",
                    message="test", raw="test", metadata={})
